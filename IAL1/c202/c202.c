...
void Stack_Init(Stack* stack) {
	...
	// This funtion sets topIndex to -1 in case that pointer exists
	if (stack == NULL)
		Stack_Error(SERR_INIT);

	// Empty stack means that topIndex value is -1
	else
		stack -> topIndex = -1;
}
...
int Stack_IsEmpty(const Stack* stack) {
	...
	// Empty stack -> topIndex is -1
	// This function returns boolean value if topIndex == -1
...
int Stack_IsFull(const Stack* stack) {
	...
	// Maximum index is 'STACK_SIZE'
	// This function returns boolean value if topIndex == STACK_SIZE
...
void Stack_Top(const Stack* stack, char* dataPtr) {
	...
	// Store last element (last index is topIndex) to pointer *dataPtr
	else
...
void Stack_Pop(Stack* stack) {
	...
	// If it's possible delete top element - don't call error function
	// topIndex is the last index - so we decrement it by 1
	if (!Stack_IsEmpty(stack))
		stack -> topIndex--;
}
...
void Stack_Push(Stack* stack, char data) {
	...
	if (Stack_IsFull(stack))
		...
		// Add an item - increment index by 1 and store data at current position
		stack->topIndex++;
